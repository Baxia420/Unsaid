...
describe('code-owned outcomes', () => {
  ...
  it('allows meaningful recovery after early mistakes', () => {
    const impacts = [...Array(5).fill('defense'), ...Array(5).fill('acknowledgment'), ...Array(5).fill('repair')] as PerceivedImpact[];
  ...
  it('lets late harm damage an otherwise constructive run', () => {
    const impacts = [...Array(11).fill('repair'), 'pressure', 'defense', 'minimization', 'pressure'] as PerceivedImpact[];
  ...
  it('does not let one final line determine the whole outcome', () => {
    const impacts = [...Array(14).fill('pressure'), 'repair'] as PerceivedImpact[];
  ...
  it('uses emotional state deterministically', () => {
    const assessments = sequence(Array(15).fill('unclear'));
